...
export class ControllerFilterPipe implements PipeTransform {
  transform(allControllerList: Controller[], searchText: string, selectedArea: Area[]): Controller[] {
    ...
    if (searchText) {
      ...
      controllerList = allControllerList.filter((controller) => {
        ...
        result =
          (controller.areaName && controller.areaName.toLowerCase().indexOf(filter) >= 0) ||
          ...
          controller.actionList.filter((action) => {
            return (
              action.actionName.toLowerCase().indexOf(filter) >= 0 ||
